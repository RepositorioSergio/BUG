...
$string = file_get_contents("benchmark_cities.json");
$response = json_decode($string, true);
foreach ($response as $key => $value) {
    $benchmarkCitiesId = $value['benchmarkCitiesId'];
    ...
    try {               
        $sql = new Sql($db);
        $insert = $sql->insert();
    $insert->into('benchmark_cities');
        $insert->values(array(
            'benchmarkcitiesid' => $benchmarkCitiesId,
