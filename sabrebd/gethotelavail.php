...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU GET HOTEL AVAIL<br/>";
...
<SOAP-ENV:Header>
    <eb:MessageHeader eb:version="3.0.0" xmlns:eb="http://www.ebxml.org/namespaces/messageHeader">
        ...
        <eb:Action>GetHotelAvailRQ</eb:Action>
        <eb:MessageData>
            <eb:MessageId>LbQ26Jnofb4Q8f3Pk15Mg5</eb:MessageId>
            <eb:Timestamp>2020-04-14T09:10:14</eb:Timestamp>
        </eb:MessageData>
            </eb:MessageHeader>
        <wsse:Security xmlns:wsse="http://schemas.xmlsoap.org/ws/2002/12/secext">
        <wsse:BinarySecurityToken>Shared/IDL:IceSess\/SessMgr:1\.0.IDL/Common/!ICESMS\/ACPCRTC!ICESMSLB\/CRT.LB!1586855413964!3394!9</wsse:BinarySecurityToken>
    </wsse:Security>
</SOAP-ENV:Header>
<SOAP-ENV:Body>
   <GetHotelAvailRQ xmlns="http://services.sabre.com/hotel/avail/v3_0_0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="3.0.0" xsi:schemaLocation="http://services.sabre.com/hotel/avail/v3_0_0 GetHotelAvailRQ_v3.0.0.xsd">
  <SearchCriteria OffSet="2" SortBy="TotalRate" SortOrder="DESC" PageSize="200" TierLabels="true" RateDetailsInd="true">
            <GeoSearch>
                <GeoRef Radius="100" UOM="MI">
                    <RefPoint Value="Irving" ValueContext="NAME" RefPointType="5" CountryCode="US"/>
                </GeoRef>
            </GeoSearch>
            <RateInfoRef CurrencyCode="USD" BestOnly="1" PrepaidQualifier="IncludePrepaid">
                <StayDateRange StartDate="2020-11-22" EndDate="2020-11-24"/>
                <RateRange Min="100.0" Max="1000.0"/>
                <Rooms>
                ...
                <RatePlanCandidates ExactMatchOnly="false">
                    <RatePlanCandidate RatePlanType="18" RatePlanCode="11"/>
                </RatePlanCandidates>
                -->
            ...
            <ImageRef Type="ORIGINAL" LanguageCode="EN"/>
        </SearchCriteria>
    </GetHotelAvailRQ>
</SOAP-ENV:Body>
</SOAP-ENV:Envelope>';
...
$GetHotelAvailRS = $Body->item(0)->getElementsByTagName("GetHotelAvailRS");
if ($GetHotelAvailRS->length > 0) {
    $HotelAvailInfos = $GetHotelAvailRS->item(0)->getElementsByTagName("HotelAvailInfos");
    if ($HotelAvailInfos->length > 0) {
        $HotelAvailInfo = $HotelAvailInfos->item(0)->getElementsByTagName("HotelAvailInfo");
        if ($HotelAvailInfo->length > 0) {
            for ($i=0; $i < $HotelAvailInfo->length; $i++) { 
                $HotelInfo = $HotelAvailInfo->item($i)->getElementsByTagName("HotelInfo");
                if ($HotelInfo->length > 0) {
                    ...
                    $SabreRating = $HotelInfo->item(0)->getAttribute("SabreRating");
                    $Direction = $HotelInfo->item(0)->getAttribute("Direction");
                    $Logo = $HotelInfo->item(0)->getAttribute("Logo");
                    $UOM = $HotelInfo->item(0)->getAttribute("UOM");
                    $Ordinal = $HotelInfo->item(0)->getAttribute("Ordinal");
                    $Distance = $HotelInfo->item(0)->getAttribute("Distance");
                    ...
                    $PropertyTypeInfo = $HotelInfo->item($i)->getElementsByTagName("PropertyTypeInfo");
                    if ($PropertyTypeInfo->length > 0) {
                        $PropertyType = $PropertyTypeInfo->item(0)->getElementsByTagName("PropertyType");
                        if ($PropertyType->length > 0) {
                            for ($j=0; $j < $PropertyType->length; $j++) { 
                                $Code = $PropertyType->item($j)->getAttribute("Code");
                                $Description = $PropertyType->item($j)->getAttribute("Description");
                            }
                        }
                    }
                }
