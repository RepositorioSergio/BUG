...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU INBOUND SEARCH<br/>";
...
$url = "https://www.saveatrain.com/api/searches/e9yzCV/inbound_search";
$username = "api.xl";
$password = "JNpWAfo%3d&";
...
$return_datetime = $response['return_datetime'];
...
$inbound_results = $response['inbound_results'];
if (count($inbound_results) > 0) {
    for ($k=0; $k < count($inbound_results); $k++) { 
        $id = $inbound_results[$k]['id'];
        $departure_datetime = $inbound_results[$k]['departure_datetime'];
        $arrival_datetime = $inbound_results[$k]['arrival_datetime'];
        $duration = $inbound_results[$k]['duration'];
        $best_price = $inbound_results[$k]['best_price'];
        $kind = $inbound_results[$k]['kind'];
        $changes_count = $inbound_results[$k]['changes_count'];
        $route = $inbound_results[$k]['route'];
        $origin_station = $route['origin_station'];
        $origin_station_uid = $origin_station['uid'];
        $names = $origin_station['names'];
        for ($i=0; $i < count($names); $i++) { 
