...
if ($HotelCityCode != "" and $OceanbyH10HotelsServiceURL != "") {
    ...
    for ($zCities = 0; $zCities < count($HotelCityCode); $zCities ++) {
        ...
        $xmlrequest .= '</m:RoomStayCandidates><m:HotelSearchCriteria><m:Criterion><m:HotelRef HotelCode="" HotelCityCode="' . $HotelCityCode[$zCities] . '" ChainCode="" VendorCode="' . $VendorCode . '" /></m:Criterion></m:HotelSearchCriteria></m:AvailRequestSegment></m:AvailRequestSegments></m:OTA_HotelAvailRQ></SOAP-ENV:Body></SOAP-ENV:Envelope>';
        $client = new Client();
        ...
        $client->setUri($OceanbyH10HotelsServiceURL);
        ...
        if ($validSearch == true) {
            ...
            for ($c = 0; $c < $node->length; $c ++) {
                ...
                for ($k = 0; $k < $RoomRates->length; $k ++) {
                    ...
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['quoteid'] = md5(uniqid($session_id, true)) . "-" . $index . "-24";
                    ...
                    // Markup
                    if ($OceanbyH10HotelsMarkup != 0) {
                        $Total = $Total + (($Total * $OceanbyH10HotelsMarkup) / 100);
                    }
                    // Geo target markup
                    if ($internalmarkup != 0) {
                    ...
                    // Fallback Markup
                    if ($OceanbyH10HotelsMarkup == 0 and $internalmarkup == 0 and $agent_markup == 0) {
    ...
    if ($valid == 1) {
        ...
        if ($sfilter != "") {
            try {
                $sql = "select hid, sid from xmlhotels_mocean where " . $sfilter;
                $statement = $db->createStatement($sql);
                $statement->prepare();
            ...
            if (is_array($sidfilter)) {
                ...
                try {
                    $sql = new Sql($db);
                    $delete = $sql->delete();
                    $delete->from('quote_session_ocean');
                    $delete->where(array(
                    ...
                    $sql = new Sql($db);
                    $insert = $sql->insert();
                    $insert->into('quote_session_ocean');
                    $insert->values(array(
                        'session_id' => $session_id,
