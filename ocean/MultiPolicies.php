...
$shid = 0;
$sindex = $index;
$dbOcean = new \Zend\Db\Adapter\Adapter($config);
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_ocean where session_id='" . $session_id . "-" . $index . "'";
} else {
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_ocean where session_id='$session_id'";
}
try {
    $statement = $dbOcean->createStatement($sql);
    $statement->prepare();
...
try {
    $hotel = array();
    $sql = "select sid from xmlhotels_mocean where sid='" . $shid . "' and hid=" . $hid;
    $statement = $dbOcean->createStatement($sql);
    $statement->prepare();
...
if ($row_hotel->valid()) {
    ...
    $statement2 = $dbOcean->createStatement($sql);
    $statement2->prepare();
    try {
