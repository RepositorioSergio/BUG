...
if ($hotellist != "") {
    ...
    // error_log("\r\Riu Auth/Userpass : " . $userpass . "\r\n", 3, "/srv/www/htdocs/error_log");
    // error_log("\r\Riu Login : " . $raw2 . "\r\n", 3, "/srv/www/htdocs/error_log");
    $login = base64_encode($userpass);
    // error_log("\r\Riu Auth/Base64Encode : " . $login . "\r\n", 3, "/srv/www/htdocs/error_log");
    $startTime = microtime();
    $client1 = new Client();
    ...
    if ($failed == false) {
        ...
        if ($JSESSIONID == "") {
            ...
            for ($r = 0; $r < $rooms; $r ++) {
                ...
                $raw .= '<InfantsCount>' . $infants . '</InfantsCount></RoomStayCandidate>';
            }
            $raw .= '</RoomConfig></RoomList><ns1:RoomsCount xmlns:ns1="http://dtos.enginexml.rumbonet.riu.com">' . $rooms . '</ns1:RoomsCount><ns1:StayDateStart xmlns:ns1="http://dtos.enginexml.rumbonet.riu.com">' . strftime("%Y%m%d", $from) . '</ns1:StayDateStart><ns1:StayDateEnd xmlns:ns1="http://dtos.enginexml.rumbonet.riu.com">' . strftime("%Y%m%d", $to) . '</ns1:StayDateEnd></ns6:in0></ns6:HotelAvail></soapenv:Body></soapenv:Envelope>';
            if ($riuServiceURL != "" and $riuLoginEmail != "" and $riuPassword != "" and $JSESSIONID != "") {
                $startTime = microtime();
                $client = new Client();
