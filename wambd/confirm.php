   ...
   <pac:confirm>
      <arg0>
         ...
         <remark>
            <code>1</code>
            <type>AGE</type>
            <value>COMENTARIOS SOBRE LA RESERVA</value>
         </remark>
      </arg0>
...
if ($confirmResponse->length > 0) {
    $return = $confirmResponse->item(0)->getElementsByTagName("return");
    if ($return->length > 0) {
        $agencyReference = $return->item(0)->getElementsByTagName("agencyReference");
        ...
        $locator = $return->item(0)->getElementsByTagName("locator");
        if ($locator->length > 0) {
            $locator = $locator->item(0)->nodeValue;
        } else {
            $locator = "";
        }
        $remark = $return->item(0)->getElementsByTagName("remark");
        if ($remark->length > 0) {
            $code = $remark->item(0)->getElementsByTagName("code");
            if ($code->length > 0) {
                $code = $code->item(0)->nodeValue;
            } else {
                $code = "";
            }
            $type = $remark->item(0)->getElementsByTagName("type");
            if ($type->length > 0) {
                $type = $type->item(0)->nodeValue;
            } else {
                $type = "";
            }
            $value = $remark->item(0)->getElementsByTagName("value");
            if ($value->length > 0) {
