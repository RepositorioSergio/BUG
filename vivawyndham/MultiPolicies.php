...
$hid = 0;
$shid = 0;
$dbVivaWyndham = new \Zend\Db\Adapter\Adapter($config);
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_vivawyndham where session_id='" . $session_id . "-" . $index . "'";
} else {
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_vivawyndham where session_id='$session_id'";
}
try {
    $statement = $dbVivaWyndham->createStatement($sql);
    $statement->prepare();
...
$fromVivaWyndham = DateTime::createFromFormat("d-m-Y", $from);
$toVivaWyndham = DateTime::createFromFormat("d-m-Y", $to);
$nights = $fromVivaWyndham->diff($toVivaWyndham);
$nights = $nights->format('%a');
$fromVivaWyndham = $fromVivaWyndham->getTimestamp();
$toVivaWyndham = $toVivaWyndham->getTimestamp();
$c = $nroom;
$response = array();
...
try {
    $hotel = array();
    $sql = "select sid from xmlhotels_mvivawyndham where sid=" . $shid . " and hid=" . $hid;
    $statement = $dbVivaWyndham->createStatement($sql);
    $statement->prepare();
...
if ($row_hotel->valid()) {
    ...
    $statement2 = $dbVivaWyndham->createStatement($sql);
    $statement2->prepare();
    try {
