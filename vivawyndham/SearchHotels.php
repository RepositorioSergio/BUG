...
$sql = "select city_xml67 from cities where id=" . $destination;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $HotelCityCode = $row_settings["city_xml67"];
}
if ($nationality > 0) {
...
if ($HotelCityCode != "" and $VivaWyndhamServiceURL != "") {
    ...
    $validSearch = true;
    $startTime = microtime();
    $xmlrequest = '<?xml version="1.0" encoding="utf-8"?><SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><SOAP-ENV:Body><m:OTA_HotelAvailRQ xmlns:m="http://www.opentravel.org/OTA/2003/05" Version="1" PrimaryLangID="en" AvailRatesOnly="true"><m:POS><m:Source><m:RequestorID MessagePassword="' . $VivaWyndhampassword . '" ID="' . $VivaWyndhamUsername . '" /></m:Source></m:POS><m:AvailRequestSegments><m:AvailRequestSegment><m:StayDateRange Start="' . strftime("%Y-%m-%d", $from) . '" End="' . strftime("%Y-%m-%d", $to) . '" /><m:RoomStayCandidates>';
    $rCount = 1;
    ...
    $client->setUri($VivaWyndhamServiceURL);
    ...
    if ($valid == 1) {
        ...
        if ($sfilter != "") {
            try {
                $sql = "select hid, sid from xmlhotels_mvivawyndham where " . $sfilter;
                $statement = $db->createStatement($sql);
                $statement->prepare();
