...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU RESERVATION CANCEL<br/>";
    ...
    if ($Response->length > 0) {
        ...
        if ($Cart->length > 0) {
            ...
            if ($Comments->length > 0) {
                ...
                if ($Comment->length > 0) {
                    for ($c=0; $c < $Comment->length; $c++) { 
                        $LineNumber = $Comment->item($c)->getAttribute("LineNumber");
                        $Timestamp = $Comment->item($c)->getAttribute("Timestamp");

                        $comment = $Comment->item($c)->nodeValue;
                    }
                }
            }
