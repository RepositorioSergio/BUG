...
use Zend\Log\Logger;
use Zend\Log\Writer;
error_log("\r\n IBEROSTAR - Hotel Parallel Search - Parse\r\n", 3, "/srv/www/htdocs/error_log");
if ($response != "") {
    error_log("\r\nResponse - $response\r\n", 3, "/srv/www/htdocs/error_log");
    ...
    for ($c = 0; $c < $node->length; $c ++) {
        ...
        for ($k = 0; $k < $RoomRate->length; $k ++) {
            ...
            $rooms[$baseCounterDetails]['quoteid'] = md5(uniqid($session_id, true)) . "-" . $index . "-55";
            ...
            if ($IberstarHotelsmarkup != 0) {
                $AmountAfterTax = $AmountAfterTax + (($AmountAfterTax * $IberstarHotelsmarkup) / 100);
            }
            // Geo target markup
            if ($internalmarkup != 0) {
