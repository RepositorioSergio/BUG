...
$raw = 'xml=<request>
<auth username="' . $mundocrucerosusername . '" password="' . $mundocrucerospassword . '" />
<method action="getcruiseflights" sessionkey="' . $sessionkey . '" resultno="201_6.0" />
</request>';
...
$flight = $results->item(0)->getElementsByTagName("flight");
echo $flight->item(1)->nodeValue . " TAM<br/>";
for ($i=0; $i < $flight->length; $i++) { 
    $voo = $flight->item($i)->nodeValue;
    echo "FL: " . $voo;
    try {
        $sql = new Sql($db);
        $insert = $sql->insert();
        $insert->into('cruzeiros_flights');
        $insert->values(array(
