...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU REMOVE FROM BASKET<br/>";
...
<request>
  ...
  <method action="removefrombasket" basketkey="' . $basketkey . '" sessionkey="' . $sessionkey . '" status="Test" />
</request>';
...
if ($success == 'Y') {
    ...
    if ($request->length > 0) {
        ...
        if ($method->length > 0) {
            $basketkey = $method->item(0)->getAttribute("basketkey");
