...
if ($city_xml17 != "") {
    ...
    for ($r = 0; $r < count($selectedAdults); $r ++) {
        ...
        for ($z = 0; $z < $selectedChildren[$r]; $z ++) {
            $raw .= '<PassengerTypeQuantity Seq="' . $Seq . '" Type="CHD" Age="' . $selectedChildrenAges[$r][$z] . '" />';
            $Seq = $Seq + 1;
        }
    }
    $raw .= '</TravelerAvail>';
    ...
    for ($r = 0; $r < count($selectedAdults); $r ++) {
        ...
        $raw .= '</TravelerAvailSet><OriginDestinationInformation Type="Checkin" LocationCode="' . $city_xml17 . '" DateTime="' . strftime("%Y-%m-%d", $from) . 'T' . strftime("%H:%M:%S") . '"/><OriginDestinationInformation Type="Checkout" LocationCode="' . $city_xml17 . '" DateTime="' . strftime("%Y-%m-%d", $to) . 'T' . strftime("%H:%M:%S") . '"/></HotelAvailRQ>';
    }
    $raw .= '</Request></VAXXML>';
    // error_log("\r\nMark Service URL: $MarkInternationalURL\r\n", 3, "/srv/www/htdocs/error_log");
    // error_log("\r\nMark Request: $raw\r\n", 3, "/srv/www/htdocs/error_log");
    if ($MarkInternationalURL != "" and $MarkInternationalLogin != "" and $MarkInternationalPassword != "") {
        if ($MarkInternationalTimeout == 0) {
            $MarkInternationalTimeout = 120;
        }
        $startTime = microtime();
        $ch = curl_init();
        ...
        // ));
        // curl_setopt($ch, CURLOPT_INTERFACE, "142.44.216.146");
        ...
        $response = str_replace("&gt;", ">", $response);
        // error_log("\r\nMark International Response: $response\r\n", 3, "/srv/www/htdocs/error_log");
        curl_close($ch);
        $endTime = microtime();
        try {
            $sql = new Sql($db);
                                                    ...
                                                    if ($Price->length > 0) {
                                                        for ($pr = 0; $pr < $Price->length; $pr ++) {
                                                            ...
                                                            for ($zRooms = 0; $zRooms < count($selectedAdults); $zRooms ++) {
                                                                ...
                                                                $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['city_xml17'] = $city_xml17;
