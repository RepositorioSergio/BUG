    ...
    if ($Ships->length > 0) {
        ...
        if ($Ship->length > 0) {
            for ($j=0; $j < $Ship->length; $j++) { 
                ...
                if ($Brands->length > 0) {
                    $Brand = $Brand->item(0)->getElementsByTagName("Brand");
                    if ($Brand->length > 0) {
                ...
                if ($Decks->length > 0) {
                    ...
                    if ($Deck->length > 0) {
                        for ($jAux2=0; $jAux2 < $Deck->length; $jAux2++) { 
                            ...
                            echo "PASSOU 7";
                            $Cabins = $Deck->item($jAux2)->getElementsByTagName("Cabins");
                            if ($Cabins->length > 0) {
                                $Cabin = $Cabins->item(0)->getElementsByTagName("Cabin");
                                if ($Cabin->length > 0) {
                                    for ($jAux4=0; $jAux4 < $Cabin->length; $jAux4++) { 
                                        $Ref = $Cabin->item($jAux4)->getAttribute("Ref");
                                        echo "PASSOU 8 ";
                                        /* try {
                                            $sql = new Sql($db);
                                            $insert = $sql->insert();
                    ...
                    if ($Cabin->length > 0) {
                        ...
                        for ($jAux5=0; $jAux5 < $Cabin->length; $jAux5++) { 
                            ...
                            if ($Occupancy->length > 0) {
                                ...
                                echo $return;
                                    echo "OccupancyMax 13: " . $OccupancyMax;
                                    echo $return;
                            }
                            ...
                            if ($Dimension->length > 0) {
                                ...
                                echo $return;
                                    echo "DimensionValue 13: " . $DimensionValue;
                                    echo $return;
                            }
                                ...
                                if ($Description->length > 0) {
                                    for ($jAux3=0; $jAux3 < $Description->length; $jAux3++) { 
                                        ...
                                        try {
                                            $sql = new Sql($db);
                                            $insert = $sql->insert();
                                            $insert->into('ships_catalogue_ships_cabins_descriptions');
                                            $insert->values(array(
                                                'datetime_created' => time(),
                                                'datetime_updated' => 0,
                                                'role' => $Role,
