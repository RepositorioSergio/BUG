...
if ($Catalogue->length > 0) {
    ...
    try {
        $sql = new Sql($db);
        $insert = $sql->insert();
        $insert->into('products_catalogue');
        $insert->values(array(
                    ...
                    if ($Rating->length > 0) {
                        for ($iAux=0; $iAux < $Rating->length; $iAux++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_ratings');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'ratingid' => $ID,
                    ...
                    if ($Keyword->length > 0) {
                        ...
                        for ($iAux2=0; $iAux2 < $Keyword->length; $iAux2++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_keywords');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'keywordid' => $ID,
                    ...
                    if ($MealPlan->length > 0) {
                        for ($iAux3=0; $iAux3 < $MealPlan->length; $iAux3++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_mealplans');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'mealplanid' => $ID,
                    ...
                    if ($Type->length > 0) {
                        for ($iAux4=0; $iAux4 < $Type->length ; $iAux4++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_types');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'typeid' => $ID,
                    ...
                    if ($City->length > 0) {
                        ...
                        for ($iAux5=0; $iAux5 < $City->length; $iAux5++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_locations');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'cityid' => $ID,
                    ...
                    if ($Country->length > 0) {
                        for ($iAux6=0; $iAux6 < $Country->length; $iAux6++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_entities_countries');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'countryid' => $ID,
        ...
        if ($Segment->length > 0) {
            for ($j=0; $j < $Segment->length; $j++) { 
                ...
                try {
                    $sql = new Sql($db);
                    $insert = $sql->insert();
                    $insert->into('products_catalogue_segments');
                    $insert->values(array(
                        'datetime_created' => time(),
                        'datetime_updated' => 0,
                        'is' => $Is,
                    ...
                    if ($Code->length > 0) {
                        ...
                        for ($jAux=0; $jAux < $Code->length; $jAux++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_segments_codes');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'code' => $codeaux,
                                    ...
                                    if ($Room->length > 0) {
                                        for ($jAux6=0; $jAux6 < $Room->length; $jAux6++) { 
                                            ...
                                            try {
                                                $sql = new Sql($db);
                                                $insert = $sql->insert();
                                                $insert->into('products_catalogue_segments_segments_rooms');
                                                $insert->values(array(
                                                    'datetime_created' => time(),
                                                    'datetime_updated' => 0,
                                                    'code' => $Code,
                                    ...
                                    if ($Code->length > 0) {
                                        for ($jAux4=0; $jAux4 < $Code->length; $jAux4++) { 
                                            ...
                                            try {
                                                $sql = new Sql($db);
                                                $insert = $sql->insert();
                                                $insert->into('products_catalogue_segments_segments_codes');
                                                $insert->values(array(
                                                    'datetime_created' => time(),
                                                    'datetime_updated' => 0,
                                                    'role' => $Role,
                                    ...
                                    if ($Duration->length > 0) {
                                        for ($jAux5=0; $jAux5 < $Duration->length; $jAux5++) { 
                                            ...
                                            try {
                                                $sql = new Sql($db);
                                                $insert = $sql->insert();
                                                $insert->into('products_catalogue_segments_segments_durations');
                                                $insert->values(array(
                                                    'datetime_created' => time(),
                                                    'datetime_updated' => 0,
                                                    'value' => $Value,
                    ...
                    if ($Keyword->length > 0) {
                        for ($jAux2=0; $jAux2 < $Keyword->length; $jAux2++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_segments_keywords');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'ref' => $Ref
                    ...
                    if ($Description->length > 0) {
                        for ($jAux3=0; $jAux3 < $Description->length; $jAux3++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('products_catalogue_segments_descriptions');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'group' => $Group,
