...
$db = new \Zend\Db\Adapter\Adapter($config);
        ...
        xmlns:ns="http://www.opentravel.org/OTA/2003/05">
          <soapenv:Header />
          <soapenv:Body>
          <VehResRQ xmlns="http://www.opentravel.org/OTA/2003/05" EchoToken="1.0" Version="1.0" ReqRespVersion="large">
            <POS>
            ...
            <VehResRQCore>
                <Customer>
                    <Primary Gender="Male" BirthDate="1972-10-10" Language="EN">
                        <PersonName>
                            <GivenName>Paulo</GivenName>
                            <Surname>Andrade</Surname>
                        </PersonName>
                        <Telephone PhoneTechType="1" PhoneNumber="7854123" />
                        <Email>paulo@corp.bug-software.com</Email>
                        <Address>
                            <StreetNmbr>8345 NW 66 ST</StreetNmbr>
                ...
                <VehPref Code="hWbyvQY1" />
            </VehResRQCore>
	<VehResRQInfo>
        <ArrivalDetails TransportationCode="14" Number="6451">
          <OperatingCompany Code="AB" />
        </ArrivalDetails>
      </VehResRQInfo>
        </VehResRQ>
        </soapenv:Body>
...
$VehResRS = $Body->item(0)->getElementsByTagName('VehResRS');

//VehResRSInfo
$VehResRSInfo = $VehResRS->item(0)->getElementsByTagName('VehResRSInfo');
if ($VehResRSInfo->length > 0) {
  $TPA_Extensions = $VehResRSInfo->item(0)->getElementsByTagName("TPA_Extensions");
  if ($TPA_Extensions->length > 0) {
    ...
    try {
        $sql = new Sql($db);
        $insert = $sql->insert();
        $insert->into('reservation_TPA_Extensions');
        $insert->values(array(
...
//VehResRSCore
$VehResRSCore = $VehResRS->item(0)->getElementsByTagName('VehResRSCore');
if ($VehResRSCore->length > 0) {
    $ReservationStatus = $VehResRSCore->item(0)->getAttribute("ReservationStatus");
    ...
    if ($VehReservation->length > 0) {
        ...
        if ($VehSegmentCore->length > 0) {
            ...
            try {
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('reservation_VehSegment');
                $insert->values(array(
                    'datetime_created' => time(),
                    'datetime_updated' => 0,
            ...
            try {
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('reservation_Vehicle');
                $insert->values(array(
                    'datetime_created' => time(),
                    'datetime_updated' => 0,
                    'CodeContext' => $CodeContext,
            ...
            if ($RentalRate->length > 0) {
                ...
                try {
                    $sql = new Sql($db);
                    $insert = $sql->insert();
                    $insert->into('reservation_RentalRate');
                    $insert->values(array(
                        'datetime_created' => time(),
                        'datetime_updated' => 0,
                    ...
                    if ($VehicleCharge->length > 0) {
                        for ($i=0; $i < $VehicleCharge->length; $i++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('reservation_VehicleCharge');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
            ...
            try {
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('reservation_TotalFees');
                $insert->values(array(
                    'datetime_created' => time(),
                    'datetime_updated' => 0,
            ...
            if ($PricedCoverages->length > 0) {
                ...
                for ($k=0; $k < $PricedCoverage->length; $k++) { 
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('reservation_PricedCoverage');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
                            'Details' => $Details,
            ...
            if ($PaymentRule->length > 0) {
                for ($x=0; $x < $PaymentRule->length; $x++) { 
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('reservation_PaymentRule');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
