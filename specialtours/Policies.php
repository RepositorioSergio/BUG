...
error_log("\r\n COMECA ENABLE \r\n", 3, "/srv/www/htdocs/error_log");
$db = new \Zend\Db\Adapter\Adapter($config);
$sql = "select value from settings where name='enablespecialtours' and affiliate_id=$affiliate_id" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
if ($row_settings->valid()) {
...
$db->getDriver()
    ->getConnection()
    ->disconnect();
error_log("\r\n ENDPOINT $SpecialToursServiceURL \r\n", 3, "/srv/www/htdocs/error_log");
$breakdown = array();
