...
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_specialtours where session_id='" . $session_id . "-" . $index . "'";
} else {
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_specialtours where session_id='$session_id'";
}
try {
...
$row_settings->buffer();
if ($row_settings->valid()) {
    $affiliate_id_specialtours = $affiliate_id;
} else {
    $affiliate_id_specialtours = 0;
}
$sql = "select value from settings where name='SpecialTourslogin' and affiliate_id=$affiliate_id_specialtours" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='SpecialTourspassword' and affiliate_id=$affiliate_id_specialtours" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='SpecialToursMarkup' and affiliate_id=$affiliate_id_specialtours" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='SpecialToursServiceURL' and affiliate_id=$affiliate_id_specialtours" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='SpecialToursTimeout' and affiliate_id=$affiliate_id_specialtours" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
foreach ($breakdownTmp as $k => $v) {
    foreach ($v as $key => $value) {
        ...
        $item['RoomDescr'] = $value['RoomDescr'];
...
$hotel = array();
$sql = "select sid from xmlhotels_mspecialtours where sid='" . $shid . "' and hid=" . $hid;
// error_log("\r\n$sql\r\n", 3, "/srv/www/htdocs/error_log");
$statement = $db->createStatement($sql);
try {
    $statement->prepare();
...
error_log("\r\n SPECIALTOURS Policies Multi - EOF\r\n", 3, "/srv/www/htdocs/error_log");
?>