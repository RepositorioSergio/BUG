...
$sfilter = array();
$olympiaeurope = false;
$sql = "select city_xml75 from cities where id=" . $destination;
...
if ($city_xml75 != "") {
    ...
    $raw .= '</RoomCandidates></HotelSearch></OTA_HotelAvailRQ></soap-env:Body></soap-env:Envelope>';
    error_log("\r\nRequest: $raw \r\n", 3, "/srv/www/htdocs/error_log");
                    ...
                    if ($Rooms->length > 0) {
                        ...
                        if ($Room->length > 0) {
                            for ($iAux = 0; $iAux < $Room->length; $iAux ++) {
                                ...
                                $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['MealPlanCode'] = $MealPlan;
