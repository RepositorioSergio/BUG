...
if ($hasChildren == false) {
    ...
    if ($pais != "" and $provincia != "" and count($selectedAdults) <= 2) {
        ...
        $nC = 0;
        $multiParallel = array();
        $requestsParallel = array();
        $channelsRequest = array();
        $multiParallel = curl_multi_init();
        for ($r = 0; $r < $rooms; $r ++) {
            $xmlrequest = 'xml=<?xml version="1.0" encoding="UTF-8"?><peticion><tipo>110</tipo><nombre>Availability</nombre><agencia>BUG</agencia><parametros><pais>' . $pais . '</pais><provincia>' . $provincia . '</provincia><pais_cliente>' . $sourceMarket . '</pais_cliente><radio>9</radio><fechaentrada>' . strftime("%m/%d/%Y", $from) . '</fechaentrada><fechasalida>' . strftime("%m/%d/%Y", $to) . '</fechasalida><afiliacion>' . $RestelAffiliate . '</afiliacion><usuario>' . $RestelHotUSACustomerID . '</usuario><numhab1>1</numhab1><paxes1>' . (int) $selectedAdults[$r] . '-' . (int) $selectedChildren[$r] . '</paxes1>';
            if ((int) $selectedChildren[$r] > 0) {
                $xmlrequest .= '<edades1>';
                for ($z = 0; $z < (int) $selectedChildren[$r]; $z ++) {
                    if ($z > 0) {
                        $xmlrequest .= ',';
                    }
                    $xmlrequest .= $selectedChildrenAges[$r][$z];
                }
                $xmlrequest .= '</edades1>';
            } else {
                $xmlrequest .= '<edades1></edades1>';
            }
            $xmlrequest .= '<numhab2>0</numhab2><paxes2>0</paxes2><edades2></edades2><numhab3>0</numhab3><paxes3>0</paxes3><edades3></edades3><idioma>2</idioma><duplicidad>1</duplicidad><comprimido>2</comprimido><informacion_hotel>0</informacion_hotel></parametros></peticion>';
            $ch = curl_init();
        ...
        foreach ($channelsParallel as $zRooms => $channel) {
            ...
            $xmlrequest = $channelsRequest[$zRooms];
            ...
            for ($xHotel = 0; $xHotel < $nodeLength; $xHotel ++) {
                ...
                if ($shid->length > 0) {
                    ...
                    if ($res->length > 0) {
                        ...
                        $length = $pax->length;
                        for ($zRoomsTmp = 0; $zRoomsTmp < $length; $zRoomsTmp ++) {
                            $code = $pax->item(0)->getAttribute("cod");
                            $hab = $pax->item($zRoomsTmp)->getElementsByTagName("hab");
