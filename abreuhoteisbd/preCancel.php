...
// URL
$sql = "select value from settings where name='AbreuSERVICECANCELLATION' and affiliate_id=$affiliate_id_abreu";
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $AbreuSERVICECANCELLATION = $row_settings['value'];
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $AbreuSERVICECANCELLATION);
