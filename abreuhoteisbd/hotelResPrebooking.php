...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU HOTEL PRE BOOKING";
...
// URL
$sql = "select value from settings where name='AbreuSERVICEBOOKING' and affiliate_id=$affiliate_id_abreu";
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $AbreuSERVICEBOOKING = $row_settings['value'];
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $AbreuSERVICEBOOKING);
