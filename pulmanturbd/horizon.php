...
while ($data = fgetcsv($file, 0, ";")) {
    ...
    $cabinno = $data[2];
    $categorycode = $data[4];
    $categorydescription = $data[7];
    $capacity = $data[8];
    $cabincategoryclass = $data[10];
    $deckcode = $data[12];
    $mainlowerbedtype = $data[13];
    $additionalbedtype = $data[14];
    $spaceforbaby = $data[15];
    $cabinlocationcode = $data[22];
    /* echo $return;
    echo "deckcode: " . $deckcode;
    echo $return; */

    $sql = new Sql($db);
    $insert = $sql->insert();
    $insert->into('cabinconfiguration_horizon');
    $insert->values(array(
