...
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_gta where session_id='" . $session_id . "-" . $index . "'";
} else {
...
foreach ($breakdownTmp as $k => $v) {
    foreach ($v as $key => $value) {
        ...
        $RoomIndex = 1;
        for ($r = 0; $r < count($adt); $r ++) {
            $xmlrequestcancel .= '<PaxRoom Adults="' . $adt . '" Id="' . $value['offerid'] . '" Cots="0" RoomIndex="' . $RoomIndex . '" />';
            $RoomIndex = $RoomIndex + 1;
        }
        $xmlrequestcancel .= '</PaxRooms></SearchHotelPricePaxRequest></RequestDetails></Request>';
        error_log("\r\n RAW - $xmlrequestcancel\r\n", 3, "/srv/www/htdocs/error_log");
        $client = new Client();
