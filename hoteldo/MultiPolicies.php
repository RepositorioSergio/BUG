...
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_hoteldo where session_id='" . $session_id . "-" . $index . "'";
} else {
...
error_log("\r\n PASSOU 2 \r\n", 3, "/srv/www/htdocs/error_log");
$sql = "select value from settings where name='HotelDouser' and affiliate_id=$affiliate_id_hoteldo" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='HotelDoMarkup' and affiliate_id=$affiliate_id_hoteldo" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
$sql = "select value from settings where name='HotelDoserviceURL ' and affiliate_id=$affiliate_id_hoteldo" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
foreach ($breakdownTmp as $k => $v) {
    foreach ($v as $key => $value) {
        ...
        //
        $raw = '/GetHotelRateRules?a=' . $HotelDouser . '&ip=' . $ipaddress . '&co=' . $sourceMarket . '&c=' . $HotelDocurrency . '&d=&l=' . $l . '&rk=' . $ratekey . '&sd=' . $from_date . '&ed=' . $to_date . '&h=' . $HotelId . '&ci=' . $contractid . '&mi=' . $marketid . '&it=' . $IdInterfaceInfo . '&ri=' . $room_code . '&mp=' . $mealplan . '&r1a=' . $adt . '&r1k=' . $chd . '';
        if ($chd > 0) {
            $children_ages = explode(",", $children_ages);
            for ($w = 0; $w < $chd; $w ++) {
