...
$sfilter = array();
$hoteldo = false;
$sql = "select city_xml49 from cities where id=" . $destination;
...
if ($city_xml49 != "") {
    if ($HotelDoserviceURL != "" and $HotelDouser != "") {
        ...
        $raw = '/GetQuoteHotels?a=' . $HotelDouser . '&co=' . $sourceMarket . '&c=' . $HotelDocurrency . '&sd=' . strftime("%Y%m%d", $from) . '&ed=' . strftime("%Y%m%d", $to) . '&d=' . $city_xml49 . '&h=&rt=&mp=&r=1&r1a=' . $selectedAdults[0] . '&r1k=' . $selectedChildren[0] . '&l=' . $l;
        if ($selectedChildren[0] > 0) {
            for ($z = 0; $z < $selectedChildren[0]; $z ++) {
                $raw .= '&r1k' . ($z + 1) . 'a=' . $selectedChildrenAges[0][$z];
            }
        }
        if ($HotelDoTimeout == 0) {
            $HotelDoTimeout = 120;
        ...
        for ($rAUX = 0; $rAUX < $node->length; $rAUX ++) {
            ...
            if ($Rooms->length > 0) {
                ...
                for ($Auxk = 0; $Auxk < $Room->length; $Auxk ++) {
                    ...
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['roomid'] = $IdRoom;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['status'] = 1;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['quoteid'] = md5(uniqid($session_id, true)) . "-" . $index . "-49";
                    if ($RoomView != "") {
                        $NameRoom .= " (" . $RoomView . ")";
                    }
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['contractid'] = $Contract;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['rstatus'] = $Status;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['room'] = $NameRoom;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['ratekey'] = $RateKey;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['roomtype'] = $IdRoom;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['mealplan'] = $IdMealPlan;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['marketid'] = $MarketId;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['IdInterfaceInfo'] = $IdInterfaceInfo;
                    ...
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['snettotal'] = $Total;
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['nettotal'] = $Total;
                    if ($HotelDoMarkup != 0) {
                        $Total = $Total + (($Total * $HotelDoMarkup) / 100);
                    }
                    // Geo target markup
                    if ($internalmarkup != 0) {
                    ...
                    try {
                        ...
                        if ($row_board_mapping->valid()) {
                            ...
                            $IdMealPlan = $row_board_mapping["mapped"];
                        }
                    } catch (\Exception $e) {
                        $logger = new Logger();
                        $writer = new Writer\Stream('/srv/www/htdocs/error_log');
                    ...
                    if ($PromoId != "") {
                        ...
                        $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['specialdescription'] = $translator->translate("Special Offer") . " " . $DescriptionPromotion;
                    } else {
                        $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['special'] = false;
                        $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['specialdescription'] = "";
                    }
                    if ($AdditionalCharges != "") {
                        $DescriptionCP .= "<br/><br/>" . $AdditionalCharges;
                    }
                    if ($DesciptionPolicy != "") {
                        $DescriptionCP .= "<br/><br/>" . $DesciptionPolicy;
                    }
                    $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['cancelpolicy'] = $DescriptionCP;
                    if ($PaymentLimitDay != "") {
                        $PaymentLimitDay = explode("-", $PaymentLimitDay);
                        $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['cancelpolicy_deadline'] = mktime(0, 0, 0, $PaymentLimitDay[1], $PaymentLimitDay[2], $PaymentLimitDay[0]);
                    }
                    $hoteldo = true;
                }
            }
