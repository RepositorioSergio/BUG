...
use Zend\Log\Logger;
use Zend\Log\Writer;
error_log("\r\nHotelDO - Hotel Parallel Search - Parse\r\n", 3, "/srv/www/htdocs/error_log");
if ($response != "") {
    error_log("\r\nResponse - $response\r\n", 3, "/srv/www/htdocs/error_log");
        ...
        if ($Rooms->length > 0) {
            ...
            if ($Room->length > 0) {
                for ($Auxk = 0; $Auxk < $Room->length; $Auxk ++) {
                    ...
                    $rooms[$baseCounterDetails]['mealplan'] = $IdMealPlan;
                    $rooms[$baseCounterDetails]['marketid'] = $MarketId;
                    ...
                    $rooms[$baseCounterDetails]['rstatus'] = $Status;
                    $rooms[$baseCounterDetails]['sourceMarket'] = $sourceMarketHotelDO;
                    $rooms[$baseCounterDetails]['contractid'] = $Contract;
                    $rooms[$baseCounterDetails]['IdInterfaceInfo'] = $IdInterfaceInfo;
                    ...
                    $rooms[$baseCounterDetails]['snettotal'] = $Total;
                    $rooms[$baseCounterDetails]['nettotal'] = $Total;
                    if ($HotelDoMarkup != 0) {
