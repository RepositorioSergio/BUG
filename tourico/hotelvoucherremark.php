...
function readCSV(string $filename){
    ...
    while ($data = fgetcsv($object, 0, "|")) {
        if ($line > 0) {
            $hotelid = $data[0];
            $voucherremark = $data[1];
            $voucherremark = str_replace('"', '', $voucherremark);
            $voucherremark = mb_convert_encoding($voucherremark, "UTF-8");
            try {               
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('hotelvoucherremark');
                $insert->values(array(
                    'hotelid' => $hotelid,
...
readCSV("HotelInfo/PDS2_HotelVoucherRemark_THF.csv");
