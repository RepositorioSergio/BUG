...
$sql = "select value from settings where name='ActionTravelb2cMarkup' and affiliate_id=$affiliate_id_actiontravel" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
...
foreach ($breakdown as $k => $v) {
    foreach ($v as $key => $value) {
        ...
        $item['OCCUPANCY'] = $value['OCCUPANCY'];
