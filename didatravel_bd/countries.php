...
$url = $didatravelserviceurl . "api/staticdata/GetCountryList?\$format=json";
echo $return;
echo $url;
echo $return;
$raw = '{
...
for ($i=0; $i < $count; $i++) { 
    $CountryName_CN = $Countries[$i]['CountryName_CN'];
    $CountryName = $Countries[$i]['CountryName'];
    $ISOCountryCode = $Countries[$i]['ISOCountryCode'];

    $sql = new Sql($db);
    $select = $sql->select();
    $select->from('didatravel_countries');
    $select->where(array(
    ...
    if ($result->valid()) {
        $data = $result->current();
        $id =  $data['ISOCountryCode'];
        if ($id->length > 0) {
            $sql = new Sql($db);
            $data = array(
                'datetime_created' => time(),
                'datetime_updated' => 1,
                'CountryName_CN' => $CountryName_CN,
                'CountryName' => $CountryName,
                'ISOCountryCode' => $ISOCountryCode
                 );
                $where['ISOCountryCode = ?']  = $ISOCountryCode;
            $update = $sql->update('didatravel_countries', $data, $where);
            $db->getDriver()
            ->getConnection()
            ->disconnect();   
        } else {
            $sql = new Sql($db);
            $insert = $sql->insert();
            $insert->into('didatravel_countries');
            $insert->values(array(
                'datetime_created' => time(),
                'datetime_updated' => 0,
                'CountryName_CN' => $CountryName_CN,
