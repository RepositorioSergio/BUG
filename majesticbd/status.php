...
<SOAP-ENV:Body>
    <m:GetStatus xmlns:m = "http://www.majesticusa.com/majesticweb_xml/">
        <m:Sort></m:Sort>
    </m:GetStatus>
</SOAP-ENV:Body>
</SOAP-ENV:Envelope>';
...
$GetStatusResponse = $Body->item(0)->getElementsByTagName("GetStatusResponse");
$GetStatusResult = $GetStatusResponse->item(0)->getElementsByTagName("GetStatusResult");
$Root = $GetStatusResult->item(0)->getElementsByTagName("Root");
$node = $Root->item(0)->getElementsByTagName("Status");
echo $return;
    ...
    try {
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $where['Id = ?'] = $Id;
                $update = $sql->update('status', $data, $where);
                $db->getDriver()
                    ->getConnection()
                    ->disconnect();
            } else {
