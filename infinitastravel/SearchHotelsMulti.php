...
if ($city_xml59 != "") {
    ...
    $sql = "select value from settings where name='infinitasTimeoutL' and affiliate_id=$affiliate_id_getaroom";
    $statement = $db->createStatement($sql);
    $statement->prepare();
    $row_settings = $statement->execute();
    $row_settings->buffer();
    ...
    if ($infinitasServiceURL != "" and $infinitasID != "" and $infinitasPassword != "") {
        for ($r = 0; $r < $rooms; $r ++) {
            ...
            $url2 = $url2 . '[';
            $url2 = $url2 . '{"Guests": [';
            $url2 = $url2 . '{"AgeQualifying":0,"Age":0,"Count":' . $selectedAdults[$r] . '}';
            if ($selectedChildren[$r] > 0) {
                $url2 = $url2 . ',';
                for ($ch = 0; $ch < $selectedChildren[$r]; $ch ++) {
                    $url2 = $url2 . '{"AgeQualifying":1,"Age":' . $selectedChildrenAges[$r][$ch] . ',"Count":1},';
                }
            }
            $url2 = $url2 . ']},';
            $url2 = $url2 . ']';
                                ...
                                for ($j = 0; $j < $RoomType->length; $j ++) {
                                    ...
                                    if ($Rate->length > 0) {
                                        ...
                                        if ($RoomsRateOccupants->length > 0) {
                                            ...
                                            for ($zRoomsTmp = 0; $zRoomsTmp < $RoomRateOccupants->length; $zRoomsTmp ++) {
                                                $TotalValueRO = $RoomRateOccupants->item($zRoomsTmp)->getAttribute("TotalValue");
                                                ...
                                                $OccupantsID = $RoomRateOccupants->item($zRoomsTmp)->getAttribute("OccupantsID");
                                                // $Guest = $RoomRateOccupants->item($zRoomsTmp)->getElementsByTagName("Guest");
                                                // if ($Guest->length > 0) {
                                                ...
                                                // $AgeQualifying = "";
                                                // }
                                                $AccommodationRate = $RoomRateOccupants->item($zRoomsTmp)->getElementsByTagName("AccommodationRate");
