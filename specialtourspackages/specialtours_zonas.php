...
echo $return;
// Catalogo
$url = $specialtourspackagesserviceURL . "v1/superbuscador/zonas";
$client->setUri($url);
...
for ($i=0; $i < $count; $i++) { 
    ...
    for ($j=0; $j < count($paisesRecorrido); $j++) { 
        ...
        $sql = new Sql($db);
        $select = $sql->select();
        $select->from('circuitos_paisesRecorrido');
        $select->where(array(
            'idPais' => $idPais
        ));
        $statement = $sql->prepareStatementForSqlObject($select);
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $data = array(
                    ...
                    'idPais' => $idPais );
                    $where['idPais = ?']  = $idPais;
                $update = $sql->update('circuitos_paisesRecorrido', $data, $where);
                $db->getDriver()
                ->getConnection()
                ->disconnect();   
            } else {
    ...
    for ($k=0; $k < count($ciudadesRecorrido); $k++) { 
        ...
        $sql = new Sql($db);
        $select = $sql->select();
        $select->from('circuitos_ciudadesRecorrido');
        $select->where(array(
            'idCircuito' => $idCircuito
        ));
        $statement = $sql->prepareStatementForSqlObject($select);
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $data = array(
                    ...
                    'idPais' => $idPais );
                    $where['idCircuito = ?']  = $idCircuito;
                $update = $sql->update('circuitos_ciudadesRecorrido', $data, $where);
                $db->getDriver()
                ->getConnection()
                ->disconnect();   
            } else {
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $data = array(
                    ...
                    'idCircuito' => $idCircuito );
                    $where['idZona = ?']  = $idZona;
                $update = $sql->update('circuitos_zonas', $data, $where);
                $db->getDriver()
                ->getConnection()
                ->disconnect();   
            } else {
    ...
    for ($m=0; $m < count($productos); $m++) { 
        ...
        $sql = new Sql($db);
        $select = $sql->select();
        $select->from('circuitos_productos');
        $select->where(array(
            'idCircuito' => $idCircuito
        ));
        $statement = $sql->prepareStatementForSqlObject($select);
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $data = array(
                    ...
                    'idCircuito' => $idCircuito);
                    $where['idCircuito = ?']  = $idCircuito;
                $update = $sql->update('circuitos_productos', $data, $where);
                $db->getDriver()
                ->getConnection()
                ->disconnect();   
            } else {
    ...
    for ($n=0; $n < count($categorias); $n++) { 
        ...
        $sql = new Sql($db);
        $select = $sql->select();
        $select->from('circuitos_categorias');
        $select->where(array(
            'idCircuito' => $idCircuito
        ));
        $statement = $sql->prepareStatementForSqlObject($select);
        ...
        if ($result->valid()) {
            ...
            if ($id > 0) {
                ...
                $data = array(
                    ...
                    'idCircuito' => $idCircuito);
                    $where['idCircuito = ?']  = $idCircuito;
                $update = $sql->update('circuitos_categorias', $data, $where);
                $db->getDriver()
                ->getConnection()
                ->disconnect();   
            } else {
