...
try {
    ...
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_bonotel where session_id='$session_id'";
...
if ($row_settings->valid()) {
    ...
    $children_ages = $searchsettings['childAge'];
} else {
    $response['error'] = "Unable to handle request #2";
    return false;
}
$db = new \Zend\Db\Adapter\Adapter($config);
$sql = "select value from settings where name='enablebonotel' and affiliate_id=$affiliate_id" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
if ($row_settings->valid()) {
    $affiliate_id_bonotel = $affiliate_id;
} else {
    $affiliate_id_bonotel = 0;
}
$db->getDriver()
    ->getConnection()
    ->disconnect();
$db = new \Zend\Db\Adapter\Adapter($config);
$sql = "select value from settings where name='bonotelUsername' and affiliate_id=$affiliate_id_bonotel" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $bonotelUsername = $row_settings['value'];
}
$db->getDriver()
    ->getConnection()
    ->disconnect();
$db = new \Zend\Db\Adapter\Adapter($config);
$sql = "select value from settings where name='bonotelPassword' and affiliate_id=$affiliate_id_bonotel" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
if ($row_settings->valid()) {
...
$db->getDriver()
    ...
    ->disconnect();
$db = new \Zend\Db\Adapter\Adapter($config);
$sql = "select value from settings where name='bonotelEndPointURL' and affiliate_id=$affiliate_id_bonotel" . $branch_filter;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $bonotelEndPointURL = $row_settings['value'];
}
$db->getDriver()
    ->getConnection()
    ->disconnect();
error_log("\r\n ENDPOINT $bonotelEndPointURL \r\n", 3, "/srv/www/htdocs/error_log");
$breakdown = array();
...
$db = new \Zend\Db\Adapter\Adapter($config);
$hotel = array();
$sql = "select sid from xmlhotels_mbonotel where sid='" . $shid . "' and hid=" . $hid;
$statement = $db->createStatement($sql);
try {
    $statement->prepare();
} catch (Exception $e) {
