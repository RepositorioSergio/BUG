...
$hid = 0;
$shid = 0;
$dbBahiaPrincipe = new \Zend\Db\Adapter\Adapter($config);
if ($details == "hoteldetails") {
    // Detail level
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_bahia where session_id='" . $session_id . "-" . $index . "'";
} else {
    $sql = "select data, searchsettings, xmlrequest, xmlresult from quote_session_bahia where session_id='$session_id'";
}
try {
    $statement = $dbBahiaPrincipe->createStatement($sql);
    $statement->prepare();
...
$fromBahia = DateTime::createFromFormat("d-m-Y", $from);
$toBahia = DateTime::createFromFormat("d-m-Y", $to);
$nights = $fromBahia->diff($toBahia);
$nights = $nights->format('%a');
$fromBahia = $fromBahia->getTimestamp();
$toBahia = $toBahia->getTimestamp();
$c = $nroom;
$response = array();
...
try {
    $hotel = array();
    $sql = "select sid from xmlhotels_mbahiaprincipe where sid=" . $shid . " and hid=" . $hid;
    $statement = $dbBahiaPrincipe->createStatement($sql);
    $statement->prepare();
...
if ($row_hotel->valid()) {
    ...
    $statement2 = $dbBahiaPrincipe->createStatement($sql);
    $statement2->prepare();
    try {
