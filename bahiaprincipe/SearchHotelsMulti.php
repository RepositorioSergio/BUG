...
$sql = "select city_xml76 from cities where id=" . $destination;
$statement = $db->createStatement($sql);
$statement->prepare();
$row_settings = $statement->execute();
$row_settings->buffer();
if ($row_settings->valid()) {
    $row_settings = $row_settings->current();
    $HotelCityCode = $row_settings["city_xml76"];
}
if ($nationality > 0) {
    ...
    foreach ($channelsParallel as $zRooms => $channel) {
        ...
        for ($c = 0; $c < $node->length; $c ++) {
            ...
            for ($k = 0; $k < $RoomRates->length; $k ++) {
                ...
                $tmp[$shid]['details'][$zRooms][$baseCounterDetails]['quoteid'] = md5(uniqid($session_id, true)) . "-" . $index . "-8";
