...
if ($result instanceof ResultInterface && $result->isQueryResult()) {
    ...
    foreach ($resultSet as $row) {
        ...
        echo $return;
        echo $return;
        $client->setUri('http://wwwaps.panavision-tours.es/integracionAme/TmsApi?invoke=consultaDisponibilidad&xml=<consulta><origen>' . $origen . '</origen><idfolleto>' . $destino . '</idfolleto><idcrucero>' . $idcircuito . '</idcrucero><idbarco>' . $idbarco . '</idbarco><fecha_salida>' . $fecha_salida . '</fecha_salida><intervalo>' . $adults . '</intervalo><n_adultos>' . $adultos . '</n_adultos><n_jubilados>' . $jubilados . '</n_jubilados><n_ninos>' . $children . '</n_ninos><n_novios>' . $noivos . '</n_novios><n_cam>' . $n_cam . '</n_cam></consulta>');
        ...
        echo $response;
        echo $return; */
/*         $start = strpos($response, "<circuito>");
        $end = strpos($response, "</circuito>");
        $response = substr($response, $start, $end - $start + 11);

        $inputDoc = new DOMDocument();
        $inputDoc->loadXML($response);
        $circuito = $inputDoc->getElementsByTagName("circuito");
        $titulo = $circuito->item(0)->getElementsByTagName("titulo");
        if ($titulo->length > 0) {
        ...
        echo $titulo;
        echo $return;
        $subtitulo = $circuito->item(0)->getElementsByTagName("subtitulo");
        if ($subtitulo->length > 0) {
            $subtitulo = $subtitulo->item(0)->nodeValue;
        } else {
            $subtitulo = "";
        }
        $duracion = $circuito->item(0)->getElementsByTagName("duracion");
        if ($duracion->length > 0) {
            $duracion = $duracion->item(0)->nodeValue;
        } else {
            $duracion = "";
        }
        $precio_desde = $circuito->item(0)->getElementsByTagName("precio_desde");
        if ($precio_desde->length > 0) {
            $precio_desde = $precio_desde->item(0)->nodeValue;
        } else {
            $precio_desde = "";
        }
        $regimen = $circuito->item(0)->getElementsByTagName("regimen");
        if ($regimen->length > 0) {
            $regimen = $regimen->item(0)->nodeValue;
        } else {
            $regimen = "";
        }
        $incluye = $circuito->item(0)->getElementsByTagName("incluye");
        if ($incluye->length > 0) {
            $incluye = $incluye->item(0)->nodeValue;
        } else {
            $incluye = "";
        }
        $incluido = $circuito->item(0)->getElementsByTagName("incluido");
        if ($incluido->length > 0) {
            $incluido = $incluido->item(0)->nodeValue;
        } else {
            $incluido = "";
        }
        $pdf = $circuito->item(0)->getElementsByTagName("pdf");
        if ($pdf->length > 0) {
        ...
        try {
            $sql = new Sql($db);
            $insert = $sql->insert();
            $insert->into('descricaocircuitos');
            $insert->values(array(
        ...
        $itinerario = $circuito->item(0)->getElementsByTagName("itinerario");
        ...
        $hoteles = $circuito->item(0)->getElementsByTagName("hoteles");
        if ($hoteles->length > 0) {
