...
/*
 * $sql = "SELECT idfolleto, idcircuito FROM circuitos";
 * $statement = $db->createStatement($sql);
 * try {
 * $statement->prepare();
 * } catch (\Exception $e) {
...
if ($result instanceof ResultInterface && $result->isQueryResult()) {
    ...
    foreach ($resultSet as $row) {
        ...
        <soapenv:Header/>
        <soapenv:Body>
            <tms:descripcionCircuito soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
                <xml xsi:type="xsd:string">?</xml>
            </tms:descripcionCircuito>
        </soapenv:Body>
        </soapenv:Envelope>';
        ...
        echo $return;
        echo $return;
        $client->setUri('http://wwwaps.panavision-tours.es/integracionAme/TmsApi?invoke=descripcionCircuito&xml=<consulta><idfolleto>' . $idfolleto . '</idfolleto><idcircuito>' . $idcircuito . '</idcircuito></consulta>');
