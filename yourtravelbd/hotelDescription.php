...
$url = 'http://testxml.youtravel.com/webservicestest/get_hotel_descriptions.asp?LangID=EN&Username=xmltestme&Password=testme';
...
if ($Hotel->length > 0) {
    for ($i=0; $i < $Hotel->length; $i++) { 
        ...
        $Photos = $Hotel->item($i)->getElementsByTagName("Photos");
        if ($Photos->length > 0) {
            $Photo = $Photos->item(0)->getElementsByTagName("Photo");
            if ($Photo->length > 0) {
                $Photo = $Photo->item(0)->nodeValue;
            } else {
                $Photo = "";
            }
        }
