...
<SOAP-ENV:Body>
    ...
    <OTA_VehModifyRQ xmlns:xsi="http://www.w3.org/2008/XMLSchema-instance" Version="1.0">
        <POS>
          <Source>
        ...
        <VehModifyRQCore ModifyType="Modify">
          ...
          <VehRentalCore PickUpDateTime="2019-11-27T12:00:00" ReturnDateTime="2019-04-29T09:00:00">
...
$OTA_VehModifyRS = $Response->item(0)->getElementsByTagName("OTA_VehModifyRS");
$TID = $OTA_VehModifyRS->item(0)->getAttribute("TID");
...
if ($VehModifyRSCore->length > 0) {
    ...
    $VehReservation = $OTA_VehModifyRS->item(0)->getElementsByTagName("VehReservation");
    if ($VehReservation->length > 0) {
        ...
        if ($VehSegmentCore->length > 0) {
                    ...
                    if ($VehicleCharge->length > 0) {
                        for ($i=0; $i < $VehicleCharge->length; $i++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('modify_VehicleCharge');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'Purpose' => $Purpose,
            ...
            $sql = new Sql($db);
            $insert = $sql->insert();
            $insert->into('modify_TotalCharge');
            $insert->values(array(
                'datetime_created' => time(),
                'datetime_updated' => 0,
                'CurrencyCode' => $CurrencyCode,
                        ...
                        if ($Paragraph->length > 0) {
                            ...
                            for ($s=0; $s < $Text->length; $s++) { 
                                ...
                                try {
                                    $sql = new Sql($db);
                                    $insert = $sql->insert();
                                    $insert->into('modify_VendorMessage');
                                    $insert->values(array(
                                        'datetime_created' => time(),
                                        'datetime_updated' => 0,
