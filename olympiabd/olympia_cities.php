...
if ($result instanceof ResultInterface && $result->isQueryResult()) {
    ...
    foreach ($resultSet as $row) {
        ...
        $Cities = $ReadResponse->item(0)->getElementsByTagName('Cities');
        if ($Cities->length > 0) {
            $City = $Cities->item(0)->getElementsByTagName('City');
            if ($City->length > 0) {
                for ($i=0; $i < $City->length; $i++) { 
                    $CityCode = $City->item($i)->getAttribute('CityCode');
                    $CityName = $City->item($i)->getElementsByTagName('CityName');
                    if ($CityName->length > 0) {
                        $CityName = $CityName->item(0)->nodeValue;
                    } else {
                        $CityName = "";
                    }
                    $CountryName = $City->item($i)->getElementsByTagName('CountryName');
                    if ($CountryName->length > 0) {
                        $CountryName = $CountryName->item(0)->nodeValue;
                    } else {
                        $CountryName = "";
                    }
                    $CountryCode = $City->item($i)->getElementsByTagName('CountryCode');
                    if ($CountryCode->length > 0) {
                        $CountryCode = $CountryCode->item(0)->nodeValue;
                    } else {
                        $CountryCode = "";
                    }
                    $CountryISO = $City->item($i)->getElementsByTagName('CountryISO');
