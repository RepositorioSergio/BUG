...
$raw = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:typ="types.circuito.dtp">     <soapenv:Header/>     <soapenv:Body>        <typ:fichaCircuitosRequest>           <idClie>' . $globaliapackagesCustomerID . '</idClie>           <idFicha>4164</idFicha>           <idioma>' . $idioma . '</idioma>        </typ:fichaCircuitosRequest>     </soapenv:Body>  </soapenv:Envelope>';
echo $return;
echo $raw;
echo $return;
$client = new Client();
...
for ($iAux = 0; $iAux < $node->length; $iAux ++) {
    ...
    for ($jAux = 0; $jAux < $serviciosCircuito->length; $jAux ++) {
        ...
        $cadRes = $preciosRegimen->item(0)->getElementsByTagName("codRga");
        if ($cadRes->length > 0) {
