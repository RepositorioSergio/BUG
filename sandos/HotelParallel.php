...
$VendorCode = "";
$hotellist = "";
$sql = "select sid from xmlhotels_msandos where hid=" . $hid;
...
if ($hotellist != "") {
    ...
    $sql = "select value from settings where name='SandosTimeout' and affiliate_id=$affiliate_id_sandos" . $branch_filter;
    $statement = $db->createStatement($sql);
    $statement->prepare();
    $row_settings = $statement->execute();
    $row_settings->buffer();
    if ($row_settings->valid()) {
        $row_settings = $row_settings->current();
        $SandosTimeout = (int) $row_settings["value"];
    } else {
        $SandosTimeout = 0;
    }
    if ($SandosTimeout == 0) {
    ...
    $raw = '<?xml version="1.0" encoding="utf-8"?><SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><SOAP-ENV:Body><m:OTA_HotelAvailRQ xmlns:m="http://www.opentravel.org/OTA/2003/05" Version="1" PrimaryLangID="en" AvailRatesOnly="true"><m:POS><m:Source><m:RequestorID MessagePassword="' . $Sandospassword . '" ID="' . $SandosUsername . '" /></m:Source></m:POS><m:AvailRequestSegments><m:AvailRequestSegment><m:StayDateRange Start="' . strftime("%Y-%m-%d", $from) . '" End="' . strftime("%Y-%m-%d", $to) . '" /><m:RoomStayCandidates><m:RoomStayCandidate RPH="1" Quantity="1"><m:GuestCounts><m:GuestCount Count="' . $adults . '" AgeQualifyingCode="10" Age="30"/>';
    ...
    curl_setopt($ch, CURLOPT_URL, $SandosServiceURL);
    ...
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $SandosTimeout);
    curl_setopt($ch, CURLOPT_TIMEOUT, $SandosTimeout);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_multi_add_handle($multiParallel, $ch);
    $requestsParallel[$nC] = 'sandos';
