...
if ($cruisedestinationid > 0) {
    ...
    $raw = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sail="http://services.rccl.com/Interfaces/SailingList" xmlns:alp="http://www.opentravel.org/OTA/2003/05/alpha"><soapenv:Header/><soapenv:Body><sail:getSailingList>
        <alp:OTA_CruiseSailAvailRQ TimeStamp="2008-07-17T12:44:44.866-04:00" Target="Test" Version="1.0" SequenceNmbr="1" PrimaryLangID="en" RetransmissionIndicator="false" MoreIndicator="true" MaxResponses="50">
            <alp:POS>
                <!--1 to 10 repetitions:-->
                <alp:Source TerminalID="' . $cruisespullmanturTerminalID . '" ISOCurrency="' . $cruisespullmanturCurrency . '">
                    <alp:RequestorID ID="' . $cruisespullmanturRequestorId . '" ID_Context="AGENCY1" Type="5"/>
                    <alp:BookingChannel Type="7">
                        <alp:CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                    </alp:BookingChannel>
                </alp:Source>
                <alp:Source TerminalID="' . $cruisespullmanturTerminalID . '" ISOCurrency="' . $cruisespullmanturCurrency . '">
                    <alp:RequestorID ID="' . $cruisespullmanturRequestorId . '" ID_Context="AGENCY2" Type="5"/>
                    <alp:BookingChannel Type="7">
                        <alp:CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                    </alp:BookingChannel>
                </alp:Source>
                <alp:Source TerminalID="' . $cruisespullmanturTerminalID . '" ISOCurrency="' . $cruisespullmanturCurrency . '">
                    <alp:RequestorID ID="' . $cruisespullmanturRequestorId . '" ID_Context="AGENT1" Type="5"/>
                    <alp:BookingChannel Type="7">
                        <alp:CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                    </alp:BookingChannel>
                </alp:Source>
    ...
    // error_log("\r\nPullmantur Username: $cruisespullmanturusername\r\n", 3, "/srv/www/htdocs/error_log");
    // error_log("\r\nPullmantur Password: $cruisespullmanturpassword\r\n", 3, "/srv/www/htdocs/error_log");
    // error_log("\r\nPullmantur Request RAW - $raw\r\n", 3, "/srv/www/htdocs/error_log");
    $ch = curl_init();
    ...
    if ($xmlresult != "") {
        ...
        if ($Envelope->length > 0) {
            ...
            if ($Body->length > 0) {
                ...
                if ($getSailingListResponse->length > 0) {
                        ...
                        if ($SailingOptions->length > 0) {
                            ...
                            if ($SailingOption->length > 0) {
                                for ($i = 0; $i < $SailingOption->length; $i ++) {
                                    ...
                                    $sql = "select name, logo, seo from cruises_lines where cruises_xml10='" . $VendorCode . "'";
                                    $statement = $dbPullmantur->createStatement($sql);
                                    try {
                                        $statement->prepare();
                                    ...
                                    if ($row->valid()) {
                                        ...
                                        error_log("\r\nUnable to find ship $shipid - Pullmantur - $sql\r\n", 3, "/srv/www/htdocs/error_log");
                                        $shipname = "";
                                    ...
                                    // Itinerary
                                    //
                                    $raw4 = '<?xml version="1.0" encoding="UTF-8"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:itin="http://services.rccl.com/Interfaces/ItineraryDetail" xmlns:alp="http://www.opentravel.org/OTA/2003/05/alpha"><soapenv:Header/><soapenv:Body><itin:getItineraryDetail>
                                            ...
                                            if ($CruiseItinInfos->length > 0) {
                                                ...
                                                if ($CruiseItinInfo->length > 0) {
                                                    for ($iTmp = 0; $iTmp < $CruiseItinInfo->length; $iTmp ++) {
                                                        $PortName = $CruiseItinInfo->item($iTmp)->getAttribute("PortName");
                                                        if ($PortName != "") {
                                                        ...
                                                        if ($iTmpnformation->length > 0) {
                                                            $Text = $iTmpnformation->item(0)->getElementsByTagName("Text");
                                                            if ($Text->length > 0) {
                            ...
                            <OTA_CruiseCategoryAvailRQ Target="Test" MaxResponses="50" MoreIndicator="true" Version="2.0" SequenceNmbr="1" TimeStamp="' . strftime("%Y-%m-%dT%H:%M:%S", time()) . '" xmlns="http://www.opentravel.org/OTA/2003/05/alpha">
                                <POS>
                                    <Source ISOCurrency="' . $cruisespullmanturCurrency . '" TerminalID="' . $cruisespullmanturTerminalID . '">
                                        <RequestorID ID="' . $cruisespullmanturRequestorId . '" Type="5" ID_Context="AGENCY1"/>
                                        <BookingChannel Type="7">
                                            <CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                                        </BookingChannel>
                                    </Source>
                                    <Source ISOCurrency="' . $cruisespullmanturCurrency . '" TerminalID="' . $cruisespullmanturTerminalID . '">
                                        <RequestorID ID="' . $cruisespullmanturRequestorId . '" Type="5" ID_Context="AGENCY2"/>
                                        <BookingChannel Type="7">
                                            <CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                                        </BookingChannel>
                                    </Source>
                                    <Source ISOCurrency="' . $cruisespullmanturCurrency . '" TerminalID="' . $cruisespullmanturTerminalID . '">
                                        <RequestorID ID="' . $cruisespullmanturRequestorId . '" Type="5" ID_Context="AGENT1"/>
                                        <BookingChannel Type="7">
                                            <CompanyName CompanyShortName="' . $cruisespullmanturCompanyShortName . '"/>
                                        </BookingChannel>
                                    </Source>
                                    ...
                                    <Guest>
                                        ...
                                        // for ($r = 0; $r < count($selectedAdults); $r ++) {
                                        // $raw .= '<GuestCounts><GuestCount Age="30" Quantity="' . $selectedAdults[$r] . '"/>';
                                        // if ($selectedChildren[$r] > 0) {
                                        // for ($z = 0; $z < $selectedChildren[$r]; $z ++) {
                                        // $raw2 .= '<GuestCount Age="' . $selectedChildrenAges[$r][$z] . '" Quantity="1"/>';
                                        // }
                                        // }
                                        // $raw2 .= '</GuestCounts>';
                                        // }
                                    ...
                                    <SelectedFare FareCode="BESTRATE"/>
                                        ...
                                        if ($getCategoryListResponse->length > 0) {
                                            ...
                                            if ($OTA_CruiseCategoryAvailRS->length > 0) {
                                                ...
                                                <alp:OTA_CruisePkgAvailRQ TimeStamp="' . strftime("%Y-%m-%dT%H:%M:%S", time()) . '" Target="Test" Version="1.0" SequenceNmbr="1" PrimaryLangID="en" RetransmissionIndicator="false" MoreIndicator="true" MaxResponses="50">
                                                    <alp:POS>
                                                        <!--1 to 10 repetitions:-->
                                                        <alp:Source TerminalID="' . $cruisespullmanturTerminalID . '" ISOCurrency="' . $cruisespullmanturCurrency . '">
                                                            <alp:RequestorID ID="' . $cruisespullmanturRequestorId . '" ID_Context="AGENCY1" Type="5"/>
                                            ...
                                            </pac:getPackageList></soapenv:Body></soapenv:Envelope>';
                                                        $ch3 = curl_init();
