...
$GetCountriesByAgencyResponse = $Body->item(0)->getElementsByTagName("GetCountriesByAgencyResponse");
if ($GetCountriesByAgencyResponse->length > 0) {
    $result = $GetCountriesByAgencyResponse->item(0)->getElementsByTagName("result");
    if ($result->length > 0) {
        ...
        if ($list->length > 0) {
            for ($i=0; $i < $list->length; $i++) { 
                ...
                try {
                    $sql = new Sql($db);
                    $select = $sql->select();
                    $select->from('transfers_countries');
                    $select->where(array(
                        'id' => $code
                    ));
                    ...
                    if ($result->valid()) {
                        ...
                        if ($id != "") {
                            ...
                            $update = $sql->update();
                            $update->table('transfers_countries');
                            $update->set($data);
                            ...
                            $sql = new Sql($db);
                            $insert = $sql->insert();
                            $insert->into('transfers_countries');
                            $insert->values(array(
                                'id' => $code,
                                'datetime_updated' => time(),
                                'name' => $name
