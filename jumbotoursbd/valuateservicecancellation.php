...
use Zend\Log\Logger;
use Zend\Log\Writer;
echo "COMECOU VALUATE SERVICE CANCELLATION<br/>";
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://xtravelsystem.com/v1_0rc1/basket/types" xmlns:xs="http://www.w3.org/2001/XMLSchema">
   <soap:Body>
      <tns:valuateServiceCancellation>
         <ValuateServiceCancellationRQ_1>
            <agencyCode>536</agencyCode>
            <brandCode>1</brandCode>
            <pointOfSaleId>1</pointOfSaleId>
            <serviceId>77504</serviceId>
         </ValuateServiceCancellationRQ_1>
      </tns:valuateServiceCancellation>
   </soap:Body>
</soap:Envelope>';
...
$valuateServiceCancellationResponse = $Body->item(0)->getElementsByTagName("valuateServiceCancellationResponse");
if ($valuateServiceCancellationResponse->length > 0) {
    $result = $valuateServiceCancellationResponse->item(0)->getElementsByTagName("result");
    if ($result->length > 0) {
        ...
        if ($hotels->length > 0) {
            for ($i=0; $i < $hotels->length; $i++) { 
                $cancellationDate = $hotels->item($i)->getElementsByTagName("cancellationDate");
                if ($cancellationDate->length > 0) {
                    $cancellationDate = $cancellationDate->item(0)->nodeValue;
                } else {
                    $cancellationDate = "";
                }
                $commision = $hotels->item($i)->getElementsByTagName("commision");
                if ($commision->length > 0) {
                    $commision = $commision->item(0)->nodeValue;
                } else {
                    $commision = "";
                }
                $contractId = $hotels->item($i)->getElementsByTagName("contractId");
                if ($contractId->length > 0) {
                    $contractId = $contractId->item(0)->nodeValue;
                } else {
                    $contractId = "";
                }
                $description = $hotels->item($i)->getElementsByTagName("description");
                if ($description->length > 0) {
                    $description = $description->item(0)->nodeValue;
                } else {
                    $description = "";
                }
                $okDate = $hotels->item($i)->getElementsByTagName("okDate");
                if ($okDate->length > 0) {
                    $okDate = $okDate->item(0)->nodeValue;
                } else {
                    $okDate = "";
                }
                $pointOfSaleId = $hotels->item($i)->getElementsByTagName("pointOfSaleId");
                if ($pointOfSaleId->length > 0) {
                    $pointOfSaleId = $pointOfSaleId->item(0)->nodeValue;
                } else {
                    $pointOfSaleId = "";
                }
                $positionInBasket = $hotels->item($i)->getElementsByTagName("positionInBasket");
                if ($positionInBasket->length > 0) {
                    $positionInBasket = $positionInBasket->item(0)->nodeValue;
                } else {
                    $positionInBasket = "";
                }
                $requestDate = $hotels->item($i)->getElementsByTagName("requestDate");
                if ($requestDate->length > 0) {
                    $requestDate = $requestDate->item(0)->nodeValue;
                } else {
                    $requestDate = "";
                }
                $serviceId = $hotels->item($i)->getElementsByTagName("serviceId");
                if ($serviceId->length > 0) {
                    $serviceId = $serviceId->item(0)->nodeValue;
                } else {
                    $serviceId = "";
                }
                $status = $hotels->item($i)->getElementsByTagName("status");
                if ($status->length > 0) {
                    $status = $status->item(0)->nodeValue;
                } else {
                    $status = "";
                }
                $userId = $hotels->item($i)->getElementsByTagName("userId");
                if ($userId->length > 0) {
                    $userId = $userId->item(0)->nodeValue;
                } else {
                    $userId = "";
                }
                $voucherUrl = $hotels->item($i)->getElementsByTagName("voucherUrl");
                if ($voucherUrl->length > 0) {
                    $voucherUrl = $voucherUrl->item(0)->nodeValue;
                } else {
                    $voucherUrl = "";
                }
                $checkin = $hotels->item($i)->getElementsByTagName("checkin");
                if ($checkin->length > 0) {
                    $checkin = $checkin->item(0)->nodeValue;
                } else {
                    $checkin = "";
                }
                $checkout = $hotels->item($i)->getElementsByTagName("checkout");
                if ($checkout->length > 0) {
                    $checkout = $checkout->item(0)->nodeValue;
                } else {
                    $checkout = "";
                }
                $establishmentId = $hotels->item($i)->getElementsByTagName("establishmentId");
                if ($establishmentId->length > 0) {
                    $establishmentId = $establishmentId->item(0)->nodeValue;
                } else {
                    $establishmentId = "";
                }
                $basket = $hotels->item($i)->getElementsByTagName("basket");
                if ($basket->length > 0) {
                ...
                $total = $hotels->item($i)->getElementsByTagName("total");
                if ($total->length > 0) {
                    $total_currencyCode = $total->item(0)->getElementsByTagName("currencyCode");
                    if ($total_currencyCode->length > 0) {
                        $total_currencyCode = $total_currencyCode->item(0)->nodeValue;
                ...
                $valuationLines = $hotels->item($i)->getElementsByTagName("valuationLines");
                ...
                $occupations = $hotels->item($i)->getElementsByTagName("occupations");
