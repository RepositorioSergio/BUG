...
if ($city_xml64 != "" or $iata_code != "") {
    ...
    if ($omnibeesServiceURL != "") {
        try {
            $client = new SoapClient($omnibeesServiceURL, array(
        ...
        $params = array();
        $params['login']['UserName'] = $omnibeesLoginEmail;
        $params['login']['Password'] = $omnibeesPassword;
        $params['ota_HotelAvailRQ']['PrimaryLangID'] = "en";
        $params['ota_HotelAvailRQ']['EchoToken'] = sha1(mt_rand(1, 90000) . 'SALT');
        $params['ota_HotelAvailRQ']['TimeStamp'] = strftime("%Y-%m-%dT%H:%m:%S", time());
        $params['ota_HotelAvailRQ']['Target'] = "Production";
        $params['ota_HotelAvailRQ']['Version'] = "2.6";
        $params['ota_HotelAvailRQ']['BestOnly'] = false;
        //
        // Hotel Code Search
        //
        // $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['HotelRefs']['HotelRef']['HotelCode'] = 474;
        //
        // City Search Code
        //
        // CountryCode
        // StateProvCode
        // <d4p1:AddressLine i:nil="true"/>
        // <d4p1:CityName i:nil="true"/>
        // <d4p1:CountryName i:nil="true"/>
        // <d4p1:PostalCode i:nil="true"/>
        // <d4p1:StateProv i:nil="true"/>
        // <d4p1:ZoneCode i:nil="true"/>
        // <d4p1:ZoneName i:nil="true"/>
        //
        if ($city_xml64 != "") {
            $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['Address']['CityCode'] = $city_xml64;
        } else {
            $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['Location']['CodeContex'] = "IATA";
            $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['Location']['LocationCode'] = $iata_code;
        }
        //
        // Adult
        //
        ...
        $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['RoomStayCandidatesType']['RoomStayCandidates']['RoomStayCandidate']['GuestCountsType']['GuestCounts']['GuestCount'][0]['Count'] = $selectedAdults[0];
        //
        // Child
        //
        if ($selectedChildren[0] > 0) {
            ...
            for ($z = 0; $z < $selectedChildren[0]; $z ++) {
                $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['RoomStayCandidatesType']['RoomStayCandidates']['RoomStayCandidate']['GuestCountsType']['GuestCounts']['GuestCount'][$GuestCount]['Age'] = $selectedChildrenAges[0][$z];
                $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['RoomStayCandidatesType']['RoomStayCandidates']['RoomStayCandidate']['GuestCountsType']['GuestCounts']['GuestCount'][$GuestCount]['AgeQualifyCode'] = "Child";
                $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['RoomStayCandidatesType']['RoomStayCandidates']['RoomStayCandidate']['GuestCountsType']['GuestCounts']['GuestCount'][$GuestCount]['Count'] = 1;
                $GuestCount ++;
            }
        }
        //
        ...
        // Date Range
        $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['StayDateRange']['Start'] = strftime("%Y-%m-%dT00:00:00", $from);
        $params['ota_HotelAvailRQ']['HotelSearchCriteria']['Criterion']['StayDateRange']['End'] = strftime("%Y-%m-%dT00:00:00", $to);
        // Get Hotel Avail
        try {
        ...
        // error_log("\r\nOmnibees Request: $xmlrequest\r\n", 3, "/srv/www/htdocs/error_log");
        // error_log("\r\nOmnibees Response: $response\r\n", 3, "/srv/www/htdocs/error_log");
        try {
            $sql = new Sql($db);
            $insert = $sql->insert();
