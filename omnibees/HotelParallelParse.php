...
use Zend\Log\Logger;
use Zend\Log\Writer;
error_log("\r\n OMNIBEES - Hotel Parallel Search - Parse\r\n", 3, "/srv/www/htdocs/error_log");
if ($response != "") {
    error_log("\r\nResponse - $response\r\n", 3, "/srv/www/htdocs/error_log");
                            ...
                            if ($node->length > 0) {
                                for ($x = 0; $x < $node->length; $x ++) {
                                    ...
                                    for ($z = 0; $z < $RoomRate->length; $z ++) {
                                        ...
                                        $rooms[$baseCounterDetails]['quoteid'] = md5(uniqid($session_id, true)) . "-" . $index . "-64";
                                        $rooms[$baseCounterDetails]['room'] = $Rooms[$RoomID]['RoomName'] . " - " . $RatePlans[$RatePlanID]['RatePlanName'];
                                        $rooms[$baseCounterDetails]['roomtype'] = $RoomTypeCode;
                                        $rooms[$baseCounterDetails]['room_description'] = $$Rooms[$RoomID]['RoomDescription'];;
                                        $rooms[$baseCounterDetails]['rateplanid'] = $RatePlanID;
                                        $rooms[$baseCounterDetails]['rateplanname'] = $RatePlans[$RatePlanID]['RatePlanName'];
                                        $rooms[$baseCounterDetails]['rateplantypecode'] = $RatePlanTypeCode;
                                        $rooms[$baseCounterDetails]['chaincode'] = $ChainCode;
                                        $rooms[$baseCounterDetails]['cancellationType'] = $Type;
                                        $rooms[$baseCounterDetails]['adults'] = $adults;
                                        $rooms[$baseCounterDetails]['children'] = $children;
                                        $rooms[$baseCounterDetails]['nettotal'] = (double) $AmountAfterTaxNet;
                                        if ($omnibeesMarkup != 0) {
                                        ...
                                        $rooms[$baseCounterDetails]['meal'] = $translator->translate($meal);
                                        $rooms[$baseCounterDetails]['mealinfo'] = $RatePlans[$RatePlanID]['MealsIncluded']['Description'];
                                        ...
                                        if ($RatePlans[$RatePlanID]['Offers'][0]['OfferDescription'] != "") {
                                            ...
                                            $rooms[$baseCounterDetails]['specialdescription'] = $RatePlans[$RatePlanID]['Offers'][0]['OfferDescription'];
                                            if ($RatePlans[$RatePlanID]['Offers'][0]['Percent'] != "") {
                                                $rooms[$baseCounterDetails]['specialdescription'] .= " - " . $RatePlans[$RatePlanID]['Offers'][0]['Percent'] . "% " . $translator->translate("discount");
                                            }
                                        } else {
                                        ...
                                        if ($RatePlans[$RatePlanID]['CancelPenalties'][0]['NonRefundable'] == true) {
                                            ...
                                            $rooms[$baseCounterDetails]['cancelpolicy'] = $translator->translate("This is a non refundable booking.") . "<br/>" . $RatePlans[$RatePlanID]['CancelPenalties'][0]['PenaltyDescription'];
                                            $rooms[$baseCounterDetails]['cancelpolicy_deadline'] = time();
                                        } else {
                                            $rooms[$baseCounterDetails]['nonrefundable'] = false;
                                            $rooms[$baseCounterDetails]['cancelpolicy'] = $RatePlans[$RatePlanID]['CancelPenalties'][0]['PenaltyDescription'];
                                            $rooms[$baseCounterDetails]['cancelpolicy_deadline'] = $RatePlans[$RatePlanID]['CancelPenalties'][0]['cancelpolicy_deadline'];
                                        }
