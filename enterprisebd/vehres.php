...
​<soapenv:Header/>
​<soapenv:Body>
    <OTA_VehResRQ PrimaryLangID="EN" TimeStamp="2019-07-10T09:54:48" Target="Production" Version="3.0" TransactionIdentifier="100000001" SequenceNmbr="1" xmlns="http://www.opentravel.org/OTA/2003/05" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.opentravel.org/OTA/2003/05 file:///C:/Users/Documents/XML/2012b%20Updated/OTA_VehResRQ.xsd">
        <POS>
        ...
        <VehResRQCore Status="Available">
            <VehRentalCore PickUpDateTime="2019-12-01T09:30:00" ReturnDateTime="2019-12-02T09:30:00">
            ...
            <Customer>
                <Primary>
                    <PersonName>
                        ...
                        <GivenName>Alamo</GivenName>
                        <Surname>Testing</Surname>
                        <NameSuffix>Sr</NameSuffix>
                    </PersonName>
                    <Telephone PhoneUseType="3" AreaCityCode="918" PhoneNumber="4016170"/>
                    <Email>linksupport@ehi.com</Email>
                    <Address>
                        <AddressLine>123 Main Street</AddressLine>
                        <AddressLine>Suite B</AddressLine>
                        <CityName>Tulsa</CityName>
                        <PostalCode>74017</PostalCode>
            ...
            <VendorPref Code="AL"/>
            <VehPref AirConditionInd="true" TransmissionType="Automatic" DriveType="Unspecified" FuelType="Unspecified">
                <VehType VehicleCategory="1"/>
                <VehClass Size="3"/>
            </VehPref>
            <DriverType Age="25"/>
            <RateQualifier RateQualifier="BQD"/>
            <SpecialEquipPrefs>
...
$OTA_VehResRS = $search->item(0)->getElementsByTagName("OTA_VehResRS");
//LocationDetail
$VehResRSCore = $OTA_VehResRS->item(0)->getElementsByTagName("VehResRSCore");
if ($VehResRSCore->length > 0) {
    $VehReservation = $VehResRSCore->item(0)->getElementsByTagName("VehReservation");
    if ($VehReservation->length > 0) {
                    ...
                    if ($AddressLine->length > 0) {
                        for ($a=0; $a < $AddressLine->length; $a++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('addressLine_vehres');
                                $insert->values(array(
        ...
        if ($VehSegmentCore->length > 0) {
            ...
            try {
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('vehSegmentCore_vehres');
                $insert->values(array(
                    'datetime_created' => time(),
                    'datetime_updated' => 0,
                    'IDConfID' => $IDConfID,
                    ...
                    if ($VehicleCharge->length > 0) {
                        for ($i = 0; $i < $VehicleCharge->length; $i ++) {
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('vehiclecharge_vehres');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'Amount' => $Amount,
            ...
            if ($PricedEquips->length > 0) {
                ...
                if ($PricedEquip->length > 0) {
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('pricedEquips_vehres');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
                            'Required' => $Required,
            ...
            if ($PricedCoverages->length > 0) {
                ...
                if ($PricedCoverage->length > 0) {
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('vehSegmentInfo_vehres');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
                            'Required' => $Required,
                ...
                if ($VendorMessage->length > 0) {
                    for ($t=0; $t < $VendorMessage->length; $t++) { 
                        ...
                        try {
                            $sql = new Sql($db);
                            $insert = $sql->insert();
                            $insert->into('vendormessages_vehres');
                            $insert->values(array(
                                'datetime_created' => time(),
                                'datetime_updated' => 0,
                                'Title' => $Title,
                        ...
                        if ($VehRentLocInfo->length > 0) {
                            for ($v=0; $v < $VehRentLocInfo->length; $v++) { 
                                ...
                                try {
                                    $sql = new Sql($db);
                                    $insert = $sql->insert();
                                    $insert->into('locationdetails_vehres');
                                    $insert->values(array(
                                        'datetime_created' => time(),
                                        'datetime_updated' => 0,
                                        'Title' => $Title,
