...
​<soapenv:Header/>
​<soapenv:Body>
<OTA_VehRetResRQ TimeStamp="2019-07-10T09:54:48" Target="Production" Version="3.0" TransactionIdentifier="100000001" SequenceNmbr="1" xmlns="http://www.opentravel.org/OTA/2003/05" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.opentravel.org/OTA/2003/05 file:///C:/Users/Documents/XML/2012b%20Updated/OTA_VehRetResRQ.xsd">
	<POS>
...
$OTA_VehRetResRS = $search->item(0)->getElementsByTagName("OTA_VehRetResRS");
//LocationDetail
    ...
    if ($VehReservation->length > 0) {
                    ...
                    if ($AddressLine->length > 0) {
                        for ($a=0; $a < $AddressLine->length; $a++) { 
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('addressLine_vehretres');
                                $insert->values(array(
        ...
        if ($VehSegmentCore->length > 0) {
            ...
            try {
                $sql = new Sql($db);
                $insert = $sql->insert();
                $insert->into('vehSegmentCore_vehretres');
                $insert->values(array(
                    'datetime_created' => time(),
                    'datetime_updated' => 0,
                    'IDConfID' => $IDConfID,
            ...
            if ($RentalRate->length > 0) {
                ...
                try {
                    $sql = new Sql($db);
                    $insert = $sql->insert();
                    $insert->into('rentalrate_vehretres');
                    $insert->values(array(
                        'datetime_created' => time(),
                        'datetime_updated' => 0,
                    ...
                    if ($VehicleCharge->length > 0) {
                        for ($i = 0; $i < $VehicleCharge->length; $i ++) {
                            ...
                            try {
                                $sql = new Sql($db);
                                $insert = $sql->insert();
                                $insert->into('vehiclecharge_vehretres');
                                $insert->values(array(
                                    'datetime_created' => time(),
                                    'datetime_updated' => 0,
                                    'Amount' => $Amount,
            ...
            if ($PricedEquips->length > 0) {
                ...
                if ($PricedEquip->length > 0) {
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('pricedEquips_vehretres');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
                            'Required' => $Required,
            ...
            if ($PricedCoverages->length > 0) {
                ...
                if ($PricedCoverage->length > 0) {
                            ...
                            if ($TaxAmount->length > 0) {
                                for ($l=0; $l < $TaxAmount->length; $l++) { 
                                    ...
                                    try {
                                        $sql = new Sql($db);
                                        $insert = $sql->insert();
                                        $insert->into('taxAmount_vehretres');
                                        $insert->values(array(
                                            'datetime_created' => time(),
                                            'datetime_updated' => 0,
                                            'Description' => $Description,
                    ...
                    try {
                        $sql = new Sql($db);
                        $insert = $sql->insert();
                        $insert->into('vehSegmentInfo_vehretres');
                        $insert->values(array(
                            'datetime_created' => time(),
                            'datetime_updated' => 0,
                            'Required' => $Required,
                ...
                if ($VendorMessage->length > 0) {
                    for ($t=0; $t < $VendorMessage->length; $t++) { 
                        ...
                        try {
                            $sql = new Sql($db);
                            $insert = $sql->insert();
                            $insert->into('vendormessages_vehretres');
                            $insert->values(array(
                                'datetime_created' => time(),
                                'datetime_updated' => 0,
                                'Title' => $Title,
                        ...
                        if ($VehRentLocInfo->length > 0) {
                            for ($v=0; $v < $VehRentLocInfo->length; $v++) { 
                                ...
                                try {
                                    $sql = new Sql($db);
                                    $insert = $sql->insert();
                                    $insert->into('locationdetails_vehretres');
                                    $insert->values(array(
                                        'datetime_created' => time(),
                                        'datetime_updated' => 0,
                                        'Title' => $Title,
...
$VehRetResRSInfo = $OTA_VehRetResRS->item(0)->getElementsByTagName("VehRetResRSInfo");
if ($VehRetResRSInfo->length > 0) {
    $TPA_Extensions = $VehRetResRSInfo->item(0)->getElementsByTagName("TPA_Extensions");
    if ($TPA_Extensions->length > 0) {
        $TPA_Extensions_Ref = $TPA_Extensions->item(0)->getElementsByTagName("TPA_Extensions_Ref");
        if ($TPA_Extensions_Ref->length > 0) {
            $PseudoCityCode = $TPA_Extensions_Ref->item(0)->getAttribute("PseudoCityCode");
        } else {
            $PseudoCityCode = "";
        }
        try {
