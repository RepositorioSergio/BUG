...
$params = array(
    ...
    "idioma" => "es",
    "expediente" => "numero expediente",//alterar
    "localizador" => "codigoSubFolleto"//alterar
);
try {
    $client->__soapCall('anulacionBonoRadial', array(
        $params
    ));
} catch (\Exception $e) {
        var_dump($e);
...
$node = $inputDoc->getElementsByTagName("anulacionBonoRadial");
$node = $inputDoc->getElementsByTagName("multiRef");

for ($j=0; $j < $node->length; $j++) { 
    $locAnulacion = $node->item($j)->getElementsByTagName('locAnulacion');
    ...
    echo $return;
    echo "locAnulacion: " . $locAnulacion;
    echo $return;
